...
app.mount("/flickr_images", StaticFiles(directory=str(flickr_data_path)), name="flickr_images")
...
model_paths = {
    "baseline": {
        "encoder": str(BASE_DIR / "models" / "model_1_baseline_cnn_lstm" / "encoder.pth"),
        "decoder": str(BASE_DIR / "models" / "model_1_baseline_cnn_lstm" / "decoder.pth"),
        "model": str(BASE_DIR / "models" / "model_1_baseline_cnn_lstm" / "model.py"),
    },
    "segmentation": {
        "encoder": str(BASE_DIR / "models" / "model_2_image_segmentation_lstm" / "encoder.pth"),
        "decoder": str(BASE_DIR / "models" / "model_2_image_segmentation_lstm" / "decoder.pth"),
        "model": str(BASE_DIR / "models" / "model_2_image_segmentation_lstm" / "model.py"),
    },
    "attention": {
        "encoder": str(BASE_DIR / "models" / "model_3_image_segmentation_attention_decoder" / "encoder.pth"),
        "decoder": str(BASE_DIR / "models" / "model_3_image_segmentation_attention_decoder" / "decoder.pth"),
        "model": str(BASE_DIR / "models" / "model_3_image_segmentation_attention_decoder" / "model.py"),
    },
    "vision_transformer": {
        "encoder": str(BASE_DIR / "models" / "model_4_vision_transformer" / "encoder.pth"),
        "decoder": str(BASE_DIR / "models" / "model_4_vision_transformer" / "decoder.pth"),
...
def generate_caption_for_image(image_path: str):
...
async def upload_form(request: Request):
    ...
    uploaded_images = [f.name for f in UPLOAD_DIR.iterdir() if f.is_file() and f.suffix.lower() in ['.png', '.jpg', '.jpeg', '.gif', '.bmp']]
...
async def generate_caption(request: Request, image_name: str = Form(...), model: str = Form(...)):
    if model in model_paths:
        load_model(model)
    else:
        model = "baseline"
