...
class EncoderCNN(nn.Module):
    ...
    def forward(self, images):
        ...
        global_features = self.global_embed(
            global_features
        ...
        detections, _ = self.roi_heads(
            features, proposals, transformed_images.image_sizes, None
        )
        ...
        for i in range(len(detections)):
            ...
            if boxes.shape[0] > 0:
                ...
                box_features = self.roi_heads.box_head(
                    box_features
                )  # Shape: (num_boxes, 1024)
                # Transform to embed_size
                object_features = self.obj_embed(
                    box_features
                )  # Shape: (num_boxes, embed_size)
                ...
                aggregated_feats = torch.zeros(self.obj_embed.out_features).to(
                    self.device
                )
        ...
        object_features = torch.stack(
            object_features_list, dim=0
        )  # Shape: (batch_size, embed_size)

        # Concatenate global and object features
        combined_features = torch.cat(
            [global_features, object_features], dim=1
        )  # Shape: (batch_size, embed_size * 2)
