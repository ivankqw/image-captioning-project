...
def preprocess_captions(captions_file):
    ...
    if not {"image", "caption"}.issubset(caption_df.columns):
        raise ValueError("The captions file must contain 'image' and 'caption' columns.")
    return caption_df
...
def initialize_model(model_name):
    ...
    if model_name == "baseline":
        ...
        from models.model_3_attention_image_segmentation_lstm.model import EncoderCNN, AttentionDecoderRNN
        encoder = EncoderCNN(embed_size)
        decoder = AttentionDecoderRNN(embed_size, hidden_size, vocab_size)
    elif model_name == "vision_transformer":
        from models.model_4_vision_transformer.model import VisionTransformerEncoder, DecoderRNN
...
def load_model(model_name):
    ...
    try:
        encoder_state_dict = torch.load(model_paths[model_name]["encoder"], map_location=torch.device("cpu"))
        decoder_state_dict = torch.load(model_paths[model_name]["decoder"], map_location=torch.device("cpu"))
